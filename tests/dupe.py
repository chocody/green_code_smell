...
class VehicleRegistry:
    ...
    def register_vehicle(self, brand: str, model: str) -> Vehicle:
        ...
        for vehicle_info in self.vehicle_models:
            ...
            if vehicle_info.model != model:
                continue
            found_info = vehicle_info
            break
        if found_info is None:
            raise VehicleInfoMissingError(brand, model)
        vehicle_id = self.generate_vehicle_id(12)
        license_plate = self.generate_vehicle_license(vehicle_id)
        new_vehicle = Vehicle(vehicle_id, license_plate, found_info)
        return new_vehicle

    def register_electric_vehicle(self, brand: str, model: str) -> Vehicle:
        """Create a new electric vehicle and generate an id and a license plate."""
        found_info = None
        for vehicle_info in self.vehicle_models:
            if vehicle_info.brand != brand:
                continue
    ...
    def register_petrol_vehicle(self, brand: str, model: str) -> Vehicle:
        """Create a new petrol vehicle and generate an id and a license plate."""
        found_info = None
        for vehicle_info in self.vehicle_models:
            if vehicle_info.brand != brand:
                continue
