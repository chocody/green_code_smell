...
class VehicleRegistry:
    ...
    def _create_vehicle_from_info(self, vehicle_info: VehicleModelInfo) -> Vehicle:
        """Helper method to create a vehicle with generated ID and license plate."""
    ...
    def register_vehicle(self, brand: str, model: str) -> Vehicle:
        ...
        return self._create_vehicle_from_info(vehicle_model)

    def register_electric_vehicle(self, brand: str, model: str) -> Vehicle:
        """Register a new electric vehicle and generates an id and a license plate."""
        if not (vehicle_model := self.find_model_info(brand, model)):
            raise VehicleInfoMissingError(brand, model)
        return self._create_vehicle_from_info(vehicle_model)

    def register_petrol_vehicle(self, brand: str, model: str) -> Vehicle:
        """Register a new petrol vehicle and generates an id and a license plate."""
        if not (vehicle_model := self.find_model_info(brand, model)):
            raise VehicleInfoMissingError(brand, model)
        return self._create_vehicle_from_info(vehicle_model)
